...
│   └── manifest.ko.webmanifest    # 한국어 설치 이름
...
- 价格单位支持 `CNY`、`HKD`、`JPY`、`USD`、`GBP`、`KRW`、`AUD`、`TWD`，下拉选项会随界面语言显示本地化名称
...
### 商品管理
- 每个活动下可创建多个商品
- 商品可编辑名称、价格、图片（上传至 R2）；价格按所选货币的主单位输入，并依据活动的 `currency_scale` 转换后以最小单位整数存储
- 图片在编辑弹窗内上传、预览、移除；销售商品卡片中的图片可点击放大查看；替换或移除时自动清理旧 R2 对象
- 商品可删除（软删除 active=0，不再显示在销售列表）
- 商品卡片分为上下两行：上层左上角显示商品缩略图，右侧信息列固定高度，商品标题最多两行并与图片顶部对齐，价格与图片底部对齐；下层数量步进器横向占满整行
...
- 侧栏展开按钮在所有尺寸常驻：桌面/平板从侧边收起或展开，手机以与平板一致的 210px 宽左侧抽屉展开；移动端展开时显示遮罩，点击侧栏外区域即可收起，不会覆盖整个页面
...
- 统计报表支持日期筛选、商品销量/退货明细、满赠发放统计和 CSV 导出；CSV 金额按活动 `currency_scale` 转换为主单位，货币单位写在金额列标题中
...
### 国际化 (i18n)
- 支持简体中文 (zh-CN)、繁体中文 (zh-TW)、粤语（香港，zh-HK）、英语 (en)、日语 (ja)、韩语 (ko)
- 默认语言偏好为“跟随系统”，按浏览器语言自动选择上述语言
- 用户可在设置中手动覆盖；顶栏语言按钮会弹出语言选择窗口，不再循环切换
...
- 系统管理员可在账号管理中编辑普通用户资料、角色及启用状态；超级管理员账号不提供编辑入口
- 修改密码时留空表示保留当前密码；填写新密码时必须再次输入并保持一致
- 用户不能禁用自己的账号或修改自己的角色；超级管理员的账号资料不能由非超级管理员修改，且其角色和启用状态对任何人都不可修改
...
| PATCH | `/api/users/:id` | 编辑普通用户资料、角色、状态或密码（管理员）；超级管理员资料仅可由超级管理员修改，角色和状态不可修改 |
...
- 价格展示和录入使用所选货币的主单位（如 `12.34 CNY`）；根据活动的 `currency_scale` 计算最小单位（`minor = round(major × 10^currency_scale)`）并以整数存储。`JPY`、`KRW`、`TWD` 默认使用 0 位小数，`GBP`、`AUD` 默认使用 2 位小数
- 订单数量允许负数，负数表示退货
